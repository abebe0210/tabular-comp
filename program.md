...
1. **run tag を決める**: 日付ベースではなく、プロジェクト名やコンペ名ベースで任意に決める（例: `tabular-comp`, `home-credit`, `santander-v1`）。ブランチ `exp/<tag>` が存在しないことを確認。
...
実験は専用ブランチ（`exp/<tag>`）で行う。`<tag>` は日付ではなく、プロジェクト名やコンペ名ベースで任意に決める。
